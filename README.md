...
* `$22$: Variable 022` Earth Shrine Flags - `word`.
  ...
  * (Flag 7 represents Llort has given his speech)
  * (Flag 8 is ???)
  * (Flag 9 is ???)
  * (Flag 10 is ???)
  * (Flag 11 is ???)
...
* `$24$: Variable 024` ZA Engine Flags - `word`.
  * (Flag 1 extend the spell reference sprite search to include region `0x8800`-`0x8FFF`. Causes slow scence load - Use with caution!)
  * (Flag 2 is ???)
  * (Flag 3 is ???)
  * (Flag 4 is ???)
  * (Flag 5 is ???)
